...
(* declaration we invert what we'd ideally like to do, which is import the *)
(* requirements into each implementation. Our logic testing relies on      *)
...
\* Next will need to be copied here to get detailed state output
Next ==
    \* Required by stub
    \/ MonthPasses
...
InTrial(u, end) ==
    \E i \in 1..end:
        ...
        (* 6.3 If the requesting User is has never been Subscribed, or     *)
...
SubscribedFromStartSubscription(u, end) ==
    (*******************************************************************)
    (* 2.4 If the requesting User is scheduled to be Not Subscribed due*)
    (*     to cancellation the requesting User SHALL remain Subscribed *)
...
CancelTrialAccessControl == 
    \A u \in USERS:
        ...
        (*******************************************************************)
        (* 8.2 [Partial] If the requesting User is In Trial the User SHALL *)
        (*     be Not Subscribed                                           *)
        (*******************************************************************) 
        \/ /\ InTrial(u, Now)
           /\ ENABLED CancelTrial(u)
...
WatchVideoAccessControl ==
    \A u \in USERS:
        ...
        (*******************************************************************)
        (* 10.2 If the requesting User is In Trial or Subscribed the system*) 
        (*      SHALL allow the User to Watch Video                        *)
        (*******************************************************************)
        \/ /\ InTrial(u, Now) \/ Subscribed(u, Now)
           /\ ENABLED WatchVideo(u)
    ...
    \A p \in paymentFailed:
        ...
        \/ /\ resubscribedAfterFailedPayment
           ...
           /\ ~\E i \in p..end:
                ...
                /\ ~\E j \in p..i:
                    j \in starts
                \* Where the user was not filled for the failed payment
...
(* 4.2.2  if the user Not Subscribed at the end of the current month they  *)
(* SHALL be Billed a Cancellation Fee                                      *)
(***************************************************************************)
...
\* Last modified Sun Jun 19 15:29:41 MST 2022 by elliotswart
